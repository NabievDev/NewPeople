...
from app.models.models import User, Category, PublicTag, InternalTag
...
def init_database():
    ...
    try:
        ...
        if db.query(PublicTag).count() == 0:
            public_tags = [
                PublicTag(name="Новое", color="#00C9C8"),
                PublicTag(name="В работе", color="#FFA500"),
                PublicTag(name="Решено", color="#22C55E"),
                PublicTag(name="Отклонено", color="#EF4444"),
            ]
        ...
        if db.query(InternalTag).count() == 0:
            internal_tags = [
                InternalTag(name="Срочно", color="#DC2626"),
                InternalTag(name="Требует проверки", color="#F59E0B"),
                InternalTag(name="Передано в департамент", color="#3B82F6"),
                InternalTag(name="Дубликат", color="#6B7280"),
                InternalTag(name="Важное", color="#8B5CF6"),
            ]

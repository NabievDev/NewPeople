...
async def get_all_tags(
    ...
    for tag in public_tags:
        all_tags.append(Tag(id=tag.id, name=tag.name, color=tag.color or "#00C9C8", is_public=True, order=0, created_at=tag.created_at))
...
async def get_public_tags(db: Session = Depends(get_db)):
    ...
    return [Tag(id=tag.id, name=tag.name, color=tag.color or "#00C9C8", is_public=True, order=0, created_at=tag.created_at) for tag in tags]
...
async def create_public_tag(
    ...
    db.commit()
    db.refresh(db_tag)
    return Tag(id=db_tag.id, name=db_tag.name, color=db_tag.color or "#00C9C8", is_public=True, order=0, created_at=db_tag.created_at)
